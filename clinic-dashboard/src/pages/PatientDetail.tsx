...
export default function PatientDetail({ clinicUser }: { clinicUser: any }) {  
  ...
  async function loadPatient() {  
    ...
    if (\!profile || profile.clinic\_id \!== clinicUser.clinic\_id) return;  
    setPatient(profile);
    ...
    const { data: proteinLogs } \= await supabase  
      .from('protein\_logs')  
      .select('amount\_g, logged\_at')  
      .eq('user\_id', id)  
      .gte('logged\_at', thirtyDaysAgo);

    const dailyMap: Record\<string, number\> \= {};  
    for (const log of (proteinLogs || \[\])) {  
      const day \= log.logged\_at.split('T')\[0\];  
      dailyMap\[day\] \= (dailyMap\[day\] || 0\) \+ log.amount\_g;  
    }

    const chart \= Object.entries(dailyMap).map((\[day, amount\]) \=\> {  
      const pct \= profile.protein\_target\_g ? (amount / profile.protein\_target\_g) \* 100 : 0;  
      let color \= '\#ef4444';  
    ...
    const { data: nudges } \= await supabase  
      ...
      .select('\*')  
      .eq('user\_id', id)  
      .gte('sent\_at', thirtyDaysAgo)  
      .order('sent\_at', { ascending: false });
  ...
  async function generateReport() {  
    ...
    const { data } \= await supabase.functions.invoke('generate-physician-report', {  
      body: { user\_id: id, month },  
    });  
    if (data?.signed\_url) {  
      window.open(data.signed\_url, '\_blank');  
    }  
  }
  ...
  return (  
    \<div style={{ minHeight: '100vh', background: '\#0f172a', padding: 40 }}\>  
      ...
      \<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}\>  
        \<div\>  
          \<h1 style={{ color: '\#f8fafc', fontSize: 32, margin: '0 0 8px' }}\>{patient.first\_name || 'Patient'}\</h1\>  
          \<p style={{ color: '\#94a3b8', margin: 0 }}\>Tier: {patient.tier} • Weight: {patient.body\_weight\_kg}kg • Protein target: {Math.round(patient.protein\_target\_g)}g\</p\>  
        \</div\>  
          ...
          \<tbody\>  
            {nudgeLog.map((n, i) \=\> (  
              \<tr key={i} style={{ borderBottom: '1px solid \#334155' }}\>  
                \<td style={{ padding: '12px 16px', color: '\#f8fafc', fontSize: 13 }}\>{new Date(n.sent\_at).toLocaleString()}\</td\>  
                \<td style={{ padding: '12px 16px', color: '\#f8fafc', fontSize: 13 }}\>{n.nudge\_type}\</td\>  
                \<td style={{ padding: '12px 16px', color: '\#f8fafc', fontSize: 13 }}\>{n.level}\</td\>  
                \<td style={{ padding: '12px 16px', fontSize: 13 }}\>  
                  \<span style={{ color: n.action\_taken \=== 'logged' ? '\#10b981' : n.action\_taken \=== 'dismissed' ? '\#f59e0b' : '\#ef4444' }}\>  
                    {n.action\_taken || 'no\_action'}  
                  \</span\>  
                \</td\>  
              \</tr\>  
            ))}  
